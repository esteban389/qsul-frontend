...
function QuestionItem({
  ...
  return (
    <motion.div
      className="flex h-full flex-col items-center justify-center gap-8 overflow-y-auto"
      ...
      <h1 className="w-full text-center font-bold text-[clamp(1.5rem,4vw,3rem)]">
        {question.text}
      </h1>
        <div className="flex items-center justify-center gap-4 py-8 flex-row flex-wrap w-full overflow-y-auto">
          {question.type === 'radio' ? (
            Array.from({ length: 5 }).map((_, i) => {
              ...
              return (
                <button
                  ...
                  className={`group p-2 md:p-0 md:ring-0 ring-1 ring-blue-600 rounded-md relative text-[4rem] transition-transform hover:scale-105 ${
                    currentAnswer === value ? 'scale-110 text-primary' : ''
                  }`}>
                  {OptionsMap[value]}
                  <span className="sr-only">{value}</span>
                  <span className="absolute hidden md:block inset-0 transition-all group-hover:blur-lg">
                    {OptionsMap[value]}
                  </span>
                </button>
              );
